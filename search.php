...
$querystart = "SELECT *, 2*LENGTH(target_sequence) - CHAR_LENGTH(REPLACE(target_sequence, \"C\", '')) - CHAR_LENGTH(REPLACE(target_sequence, \"G\", '')) AS GC_count, cleavage_experiments.delivery_mode FROM cleavage_data INNER JOIN cleavage_experiments ON cleavage_data.experiment_id = cleavage_experiments.id";
if (isset($_POST["submit_rna"]) && isset($_POST["guideid"])) {
    ...
    if (preg_match("/^(chr)[0-9XVIY]{1,2}:[0-9]{1,10}-[0-9]{1,10}$/", $targetregion)) { // verify string is a proper region
        ...
        $end  = explode("-", $temp)[1];
        $result = $conn->query("SELECT *, 2*LENGTH(target_sequence) - CHAR_LENGTH(REPLACE(target_sequence, \"C\", '')) - CHAR_LENGTH(REPLACE(target_sequence, \"G\", '')) AS GC_count, cleavage_experiments.delivery_mode FROM cleavage_data INNER JOIN cleavage_experiments ON cleavage_data.experiment_id = cleavage_experiments.id WHERE target_chr='{$chr}' AND target_start>={$start} AND target_end<={$end}");
    }
} 
if (isset($result)) {
    // track search result
    ...
    if ($num_results > 0) {
        ...
        echo '<table class="table table-striped sortable targets"><thead class="thead-dark">
              <tr>
                ...
                <th scope="col" data-toggle="tooltip" data-placement="top" data-html="true" title="GC count of<br>target sequence"><abbr title="">target GC</abbr></th>
                ...
                <th scope="col" data-toggle="tooltip" data-placement="top" data-html="true" title="EP: electroporation,<br>LF: lipofection"><abbr title="">delivery mode</abbr></th>
